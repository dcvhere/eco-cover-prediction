...
# Cached loading routine to read the local joblib file from the Space workspace
@st.cache_resource
def load_ml_pipeline():
    # Looks for the file directly in the main directory of your Hugging Face Space
    model_path = 'forest_cover_pipeline.joblib'
        
    if not os.path.exists(model_path):
        st.error(f"⚠️ Project artifact file '{model_path}' could not be discovered in the workspace root.")
        st.info("💡 Please click on the 'Files' tab at the top of your Hugging Face Space and upload your 'forest_cover_pipeline.joblib' file directly there.")
        st.stop()
